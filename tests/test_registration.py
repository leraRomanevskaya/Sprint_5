...
def test_registration_is_correct(name, random_email, random_corr_password, login_page, registration_page, driver):
...
def test_registration_uncorrect_password(name, random_email, uncorr_password, registration_page, driver):
    driver.get(registration_page)
    driver.find_element(*Locators.REGISTRATION_FIELD_NAME).send_keys(name)
    driver.find_element(*Locators.REGISTRATION_FIELD_EMAIL).send_keys(random_email)
